...
###############################################################################
# WAPT - Python 2.7 compatibility runtime for Debian 10 Buster
#
...
#
# Output:
#   /git/waptdev/build/python2-runtime-server-buster/
#
###############################################################################
...
RUNTIME_ROOT="/git/waptdev/build/python2-runtime-server-buster"
...
###############################################################################
# 4b. Buster multiarch library path fix
###############################################################################

echo
echo ">>> Applying Buster multiarch library path fix..."

if ! grep -q "'/usr/lib/x86_64-linux-gnu'," "${PYTHON_SRC}/setup.py"; then
    sed -i "/'\/usr\/lib',/a\\
                '/usr/lib/x86_64-linux-gnu'," "${PYTHON_SRC}/setup.py"
fi

grep -n -A2 -B2 "'/usr/lib/x86_64-linux-gnu'," "${PYTHON_SRC}/setup.py"
